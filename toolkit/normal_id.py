"""Sapiens 0.3B surface-normal perceptor + GT-normal caching pass.

Pure-PyTorch port of the Sapiens-0.3B normal estimator (ViT-L backbone +
ConvTranspose decoder); no ``mmseg``/``mmcv`` runtime dependency. Gradients
flow through preprocessing (differentiable letterbox + ImageNet normalize)
and the full forward, so the same instance serves the live normal loss. The
``huggingface_hub`` import lives inside ``SapiensNormal._load_pretrained`` so
selecting Krea never downloads Sapiens -- only enabling normal loss does.

GT normal maps are computed from the RAW source image (transform-independent,
unlike depth whose GT depends on the bucket transform + VAE round-trip), so
the cache identity is simpler: one file per image, version-keyed. The resident
tensor is never held on the shared file-list item; the worker re-reads lazily
via ``NormalCachingFileItemDTOMixin.get_normal_gt()``.

Loss math (cosine dissimilarity + L1) and the preview renderer live in
``toolkit.normal_id_loss``; the live decode + loss add are wired into
...
NORMAL_SIZE = 256
NORMAL_CACHE_VERSION_KEY = "normal_v1"
...
def _best_orientation(w, h):
    """Return (target_h, target_w): Sapiens native 1024x768 or 768x1024."""
    if h >= w:
        return (1024, 768)
    else:
        return (768, 1024)
...
class DifferentiableNormalEncoder(nn.Module):
    """Frozen Sapiens encoder for the normal-map auxiliary loss.
    ...
    (NORMAL_SIZE x NORMAL_SIZE), unit L2-normalized. The forward runs in fp32
    under disabled autocast but preserves the input's grad graph.
    """

    def __init__(self, device: Optional[torch.device] = None, grad_checkpoint: bool = True):
        super().__init__()
    ...
    def _letterbox_pil(pil_image, target_w, target_h):
        """Resize PIL image to fit inside target_w x target_h, padding black."""
    ...
    def _letterbox_tensor(pixels, target_h, target_w):
        """Resize (B, 3, H, W) tensor to fit inside target, padding black.
...
# ============================================================
# GT-normal caching (stamps file items for lazy worker reads)
# ============================================================

def _normal_cache_path(file_item) -> str:
    """Per-image safetensors cache file under the image dir's _normal_cache."""
    img_dir = os.path.dirname(file_item.path)
    cache_dir = os.path.join(img_dir, "_normal_cache")
    filename_no_ext = os.path.splitext(os.path.basename(file_item.path))[0]
    return os.path.join(cache_dir, f"{filename_no_ext}_normals.safetensors")


def _normal_cache_hit(cache_path: str, key: str) -> bool:
    ...
    try:
        with safe_open(cache_path, framework="pt", device="cpu") as f:
            return key in f.keys() and NORMAL_CACHE_VERSION_KEY in f.keys()
...
def cache_normal_gt(
    ...
    *,
    ...
    """Stamp every file item with its normal-cache metadata and persist GT normals.

    GT normals are computed from the RAW source image (transform-independent),
    so the cache identity is one file per image + a version key -- no bucket
    transform / VAE / arch fingerprint (unlike depth). On a header hit, skip
    recompute. The resident tensor is never held on the shared file-list item;
    the worker re-reads lazily via ``get_normal_gt()``.
    """
    from PIL import Image
    ...
    for file_item in tqdm(file_items, desc="Caching GT normal maps"):
        cache_path = _normal_cache_path(file_item)

        # stamp for the worker (hit or miss -- both point at the same file)
        file_item._normal_cache_path = cache_path
        file_item._normal_cache_key = key
        file_item.is_normal_cached = True
        file_item.normal_gt = None

        if _normal_cache_hit(cache_path, key):
            hits += 1
            continue

        misses += 1
        pil_image = exif_transpose(Image.open(file_item.path)).convert("RGB")
        normals = encoder.encode(pil_image)  # (3, NORMAL_SIZE, NORMAL_SIZE)

        if float(normals.abs().sum().item()) < 1e-6:
            zero_normals += 1

        _atomic_save_file(
            {key: normals.half(), NORMAL_CACHE_VERSION_KEY: torch.ones(1)},
            cache_path,
        )

    if hits or misses:
        print(
            f"  - GT normal cache: {hits} reused, {misses} computed "
            f"(key {key!r}). An all-computed run means a missing cache file "
            f"or a perceptor/version change."
        )
    if zero_normals > 0:
        print(f"  - Warning: zero normals for {zero_normals}/{len(file_items)} images")
