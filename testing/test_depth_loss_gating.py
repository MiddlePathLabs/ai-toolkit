...
# ----------------------------------------------------------------------
# preview_only: render on cadence without adding anchor loss or dropping diffusion
# ----------------------------------------------------------------------

def test_preview_only_zero_weight_processes_on_cadence_without_loss():
    ...
    tr.depth_consistency_config.preview_every = 2
    batch = _FakeBatch(2, loss_split_list=[None, None])
    ts = torch.tensor([500.0, 500.0])
    g = _gates(tr, batch, ts)
    assert [bool(x) for x in g['depth_objective']] == [False, False]
    assert [bool(x) for x in g.get('preview_objective', [])] == [True, True]
    assert [bool(x) for x in g['diffusion_zero']] == [False, False]

    # First preview-only step advances cadence but does not decode or run DA2.
    noise_pred = torch.zeros(2, 16, 4, 4)
    noisy = torch.zeros(2, 16, 4, 4)
    first = SDTrainer._compute_depth_anchor_loss(tr, noise_pred, noisy, ts, batch, g)
    assert float(first) == 0.0
    assert tr._depth_perceptor.calls == 0

    # Second step is due: it processes both samples for preview data but still
    # returns zero anchor contribution and does not mask diffusion.
    second = SDTrainer._compute_depth_anchor_loss(tr, noise_pred, noisy, ts, batch, g)
    assert float(second) == 0.0
    assert tr._depth_perceptor.calls == 2
...
def test_preview_only_positive_weight_suppresses_anchor_contribution():
    tr = _make_trainer(loss_weight=0.001, preview_only=True,
                       train_loss_split='diffusion_depth', explicit=True,
                       step_num=1)
    tr.depth_consistency_config.preview_every = 1
    batch = _FakeBatch(1, loss_split_list=[None])
    ts = torch.tensor([500.0])
    g = _gates(tr, batch, ts)

    assert [bool(x) for x in g['depth_objective']] == [False]
    assert [bool(x) for x in g.get('preview_objective', [])] == [True]
    assert [bool(x) for x in g['diffusion_zero']] == [False]

    noise_pred = torch.zeros(1, 16, 4, 4, requires_grad=True)
    ...
    assert noise_pred.grad is None
